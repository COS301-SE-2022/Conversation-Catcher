...
export const userSlice = createSlice({
  ...
  reducers: {
    setUser: (state, action) => {
      ...
      return state;
    }, //set the state to the action's payload
    setEmail: (state, action) => {
      state.email = action.payload;
      return state;
    }, //set the email to the action's payload
    setColour: (state, action) => {
      state.colour = action.payload;
      return state;
    }, //set the colour to the action's payload
    addPDF: (state, action) => {
      state.pdfs.push(action.payload);
      return state;
    }, //add pdf to user
    removePDF: (state, action) => {
      //remove the pdf that has the name provided in the action
...
export const { setUser, setEmail, setColour, addPDF, removePDF, clearUser } =
  actions;
